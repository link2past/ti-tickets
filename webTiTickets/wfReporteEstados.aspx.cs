...
namespace webTiTickets
{
    ...
    public partial class wfReporteEstados : Page
    // ReSharper restore InconsistentNaming
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    Util.Util.CargaEstadosTicket(cboEstado, true);
        ...
        protected void btnConsultar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var nIdEstado = cboEstado.SelectedValue.Equals("-1")
                                           ? (int?)null
                                           : Int32.Parse(cboEstado.SelectedValue);
                ...
                var oResultado = new ReporteClienteUn().Procesar(new ReporteClienteUnInfo(null, nIdCliente, null, null, dFechaDesde, dFechaHasta) { IdEstadoTicket = nIdEstado });
                Session["Reporte"] = oResultado;
                var totalEstado = from ticket in oResultado
                                     group ticket by ticket.Ticket.EstadoTicket.Descripcion
                                         into newGroup
                                         select
                                             new
                                             {
                                                 EstadoTicket = newGroup.Key,
                ...
                var totalCliente = from ticket in oResultado
                                   group ticket by new { ticket.Ticket.Cliente.RazonSocial, ticket.Ticket.EstadoTicket.Descripcion }
                                       into newGroup
                                       select
                                           new
                                           {
                                               Cliente = newGroup.Key.RazonSocial,
                                               Estado = newGroup.Key.Descripcion,
                                               Casos = newGroup.Count(),
                                               TotalRepuestos = newGroup.Sum(x => x.TotalRepuestos),
                                               TotalAtencion = newGroup.Sum(x => x.Tarifa),
                                               Suma = newGroup.Sum(x => x.TotalRepuestos + x.Tarifa)
                ...
                gvResultadoEstado.DataSource = totalEstado;
                gvResultadoEstado.DataBind();
...
: txtRazonSocialCliente.Text.Trim();
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=ReporteEstados.xls");
            Response.Charset = "";
